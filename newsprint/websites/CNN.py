class CNN(object):
    ...
    def retreive_article_data_cnn(self, json_object):
        ...
        if json_object['type'] == "article":
            ...
            # ISO 8601
            article_published_date_first = json_object['firstPublishDate']
            # ISO 8601
            article_published_date_latest = json_object['lastModifiedDate']
            try:
    ...
    def make_request(self, current_page, search_query=None):
        ...
        if search_query is None:
            search_query = ""
        start_url = base_url + "content?q=" + \
            str(search_query) + "&size=50&from=0&page=1" + \
            str(current_page) + "&sort=newest"
